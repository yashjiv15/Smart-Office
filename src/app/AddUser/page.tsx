...
import { addUser, RoleData, viewRoles } from '../../apiService';
...
const AddUser = () => {
  ...
  const [selectedRole, setSelectedRole] = useState<string>('Select user_role');
  ...
  const validateForm = () => {
    ...
    if (selectedRole === 'Select user_role') {
      newErrors.user_role = 'user_role is required';
      isValid = false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addUser(trimmedFormData);
      toast.success('User added successfully');
      window.location.href = '/ViewUser'; // Example redirect after login
    } catch (error) {
...
<form className="w-lg mx-auto mt-0 font-roboto"onSubmit={handleSubmit}> 
  ...
  <div
    id="dropdownBgHover"
    className="ml-12 z-10 absolute top-full mt-2 w-48 bg-white rounded-lg shadow dark:bg-gray-700 dropdown-menu max-h-48 overflow-y-auto"
  >
    <ul className="p-3 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownBgHoverButton">
      ...
      <div className="flex justify-center items-center font-roboto">
                ...
                <button type="reset" 
                 className="hover:shadow-form rounded-md bg-blue-600 py-3 px-5 mt-10 ml-10 text-center text-base 
                 font-semibold text-white outline-none"               onClick={resetForm}>
                ...
                <Link href="/ViewUser">
                    <button
                      type="button"
                      className="hover:shadow-form rounded-md bg-blue-600 py-3 px-5 mt-10 
                      ml-10 text-center text-base font-semibold text-white outline-none"
                    >
